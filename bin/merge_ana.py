...
#
# This function below, 'which', is from the stackoverflow link.  It
# is used here to verify that the script passed to the merger is
# in fact an available executable.
#
# As of python 3.3, there is shutil.which, and that has this functionality
# It's implemented here to allow python 2.* compatibility.
#
...
def merge(project, output_directory, file_splitting_dict, script='hadd', _type=1):
    ...
    # Verfiy the exectuable will work:
    if which(script) is None:
        raise Exception("Executable script {} either doesn't exist on the $PATH or is not executable.".format(script))
