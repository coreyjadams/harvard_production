...
def create_connection(host, username, password, db='guenette_neutrinos'):
...
def read_connection(password_file, host=None):
    if host is None:
...
def write_connection(password_file, host=None):
    if host is None:
        host = 'db-guenette_neutrinos.rc.fas.harvard.edu'
...
def admin_connection(password_file, host=None):
    if host is None:
        host = 'db-guenette_neutrinos.rc.fas.harvard.edu'
